...
#define Name_quantity 2//人數
byte chack[Name_quantity][4] = //IC卡號碼
{
  {192, 186, 145, 117},
  {245, 123, 222, 29}
};
String Name[Name_quantity] = {"sunmin", "Lushch"};//使用者名稱
String user = "";
//螢幕
#include <splash.h>
#include <Wire.h>
...
void loop() {
  ...
  if (RFID())//當RFID感測到
  {
    ...
    while (true)
    {
      ...
      if (Measurement() <= 4 and Measurement()!=0)//感測到人
      {
        int temperature = Temperature();//量測溫度
        send_N_C[1] = (String)temperature;
        //        Serial.println(user);//顯示使用者
        ...
        while ( digitalRead(above) == HIGH) //步進馬達上升
        {
          Step_motor(false);
        }
